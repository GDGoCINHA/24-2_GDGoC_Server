...
// 2026-08-05 프로브로 확정했다. 근거는 `.claude/work/harness-multirepo/verification.md`.
//
...
if (isMain) {
  ...
  // 컴파일 출력은 사용자 화면용이라 stdio: "inherit" 로 그대로 흘린다. 판정은 아래에서
  ...
  if (res.status !== 0) {
    ...
    // 2026-08-05 이전까지 컴파일 실패가 정확히 이 이유로 사라지고 있었다.
